...
**Issue:** [skiptools/skip-ui#146](https://github.com/skiptools/skip-ui/issues/146) — *closed 2026-07-01 (fixed by merged #471)*

**Fork:** [arulagarwal/skip-ui](https://github.com/arulagarwal/skip-ui)

**Status:** **Phase IV complete.** #146 was fixed by @haaaaaaarshs's merged PR [#471](https://github.com/skiptools/skip-ui/pull/471); my PR [#468](https://github.com/skiptools/skip-ui/pull/468) was closed as obviated. Taking @marcprux's invitation to look for "other improvements … (like the tests)," I opened a maintainer-invited follow-up — PR [#474](https://github.com/skiptools/skip-ui/pull/474) — adding the still-missing 8-bit (0–255) Input Method plus the fixtures/render tests #471 lacked (CI ✅, CLA ✅, awaiting review). **Second contribution cycle started** (firefox-ios — see the [index](../README.md)).
...
This contribution spans **three** PRs — my original submission, a parallel PR the maintainer merged, and a maintainer-invited follow-up I opened to round it out.

**My original PR — [skiptools/skip-ui#468](https://github.com/skiptools/skip-ui/pull/468):** opened against upstream `main` from branch [`fix-issue-146-hex-colors`](https://github.com/arulagarwal/skip-ui/tree/fix-issue-146-hex-colors). Routed each `.colorset` channel through a new `parseColorComponent` helper so **all three of Xcode's numeric Input Methods** (floating point, 8-bit hexadecimal `0xF1`/`#F1`, 8-bit 0–255) parse correctly instead of hex falling back to `0.0` (black) on Android, plus fixtures and `#if SKIP` render tests. **Status: CLOSED (2026-07-01), not merged** — closed as *obviated by #471* (below); CI ✅ and CLA ✅ at close.

**Merged fix — [#471](https://github.com/skiptools/skip-ui/pull/471) by @haaaaaaarshs:** a second CodePath contributor independently opened a leaner PR handling the `0x` hexadecimal case. The maintainer **merged #471 on 2026-07-01**, which **closed #146** and, as a side effect, closed my #468.

**Follow-up — [#474](https://github.com/skiptools/skip-ui/pull/474) (open):** after @marcprux invited me to "see if there are any other improvements … (like the tests)," I reviewed the merged parser, confirmed it covers **only** the `0x` case, and opened a scoped follow-up from branch [`colorset-input-methods`](https://github.com/arulagarwal/skip-ui/tree/colorset-input-methods) adding the still-missing **8-bit (0–255)** Input Method plus the fixtures + render tests #471 lacked. It builds on #471 ("Refs #146, builds on #471" — not `Closes`, since #146 is already closed). **Status: OPEN, `skip-framework` CI ✅, `verification/cla-signed` ✅, mergeable — awaiting @marcprux review.** Surfaced in a comment on [#146](https://github.com/skiptools/skip-ui/issues/146).

**Maintainer Feedback Log:**

| Date | Feedback | My Response | Commit / Ref |
|------|----------|-------------|--------------|
| 2026-06-22 | `cla-bot`: `@arulagarwal` not on file — `verification/cla-signed` failing on #468 | Added my username to Skip's `.clabot` via clabot-config#71; after it merged, re-ran `@cla-bot check` → CLA ✅ | [clabot-config#71](https://github.com/skiptools/clabot-config/pull/71) |
| 2026-06-23 | Surfaced #468 to @marcprux in a comment on #146 (CI + CLA green) | Awaited review | comment on #146 |
| 2026-07-01 | @marcprux **merged the parallel #471** (closing #146); my #468 auto-closed as *obviated*. He added: *"feel free to … see if there are any other improvements … (like the tests)."* | Reviewed the merged parser, confirmed it covers only the `0x` case; opened **#474** adding the missing 8-bit (0–255) method + fixtures/render tests, and surfaced it on #146 | [#474](https://github.com/skiptools/skip-ui/pull/474) |
| _(awaiting review on #474)_ | | | |
...
* Navigated the **full open-source loop**, not just submission: cleared the project's CLA gate (added my username to Skip's `.clabot` via [clabot-config#71](https://github.com/skiptools/clabot-config/pull/71), then re-ran `@cla-bot check`), kept the branch rebased on upstream `main`, and worked through a parallel PR (#471) from another contributor for the same issue. The maintainer merged the leaner #471; rather than re-litigate, I took @marcprux's invitation to build on it and opened a scoped follow-up (#474) adding the one Input Method it missed plus the tests it lacked. **The teachable insight:** on a shared issue, value isn't measured by *whose diff lands* — my thorough version informed the outcome, and once #471 merged the highest-leverage move was to complement it, not compete with it.
